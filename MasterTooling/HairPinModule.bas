...
Sub HairPin() '(UnitType As String, ByRef DimensionsArray As Variant, Optional ByRef errors As Boolean = True)
    ...
    'Coil Dimensions
    Dim HairPinCrossWidth As Double
    Dim HairPinCrossLength As Double
    Dim StackingAngle As Double
    Dim HairPinOutRadius As Double
    'Dim wireWidth As Double
    
    'Tool Dimensions
    Dim DowelPinDIA As Double
    Dim ToolOutID As Double
    Dim ToolInOD As Double
    Dim ToolOutOD As Double
...
'    End Select
    ...
    RtoCoil = DimensionsArray(0)
    NumberCoils = DimensionsArray(1)
    CoilWidth = DimensionsArray(2)
    CoilLength = DimensionsArray(3)
    CoilHeight = DimensionsArray(4)
    wireWidth = DimensionsArray(5)
    CoilRadius = DimensionsArray(6)
    
    DowelPinDIA = 0.25
    
    ToolOutID = 2 * (HairPinOutRadius - 0.001)
    ToolInOD = ToolOutID - 0.03
    ToolOutOD = Round(ToolOutID + 1.2, 1)
    ...
    Set swModel = swApp.OpenDoc6("C:\Users\FIlie\Documents\Felix Documents IPS\Master Tooling\Main rotor coils Brazing tool\Assembly, Fixture, Brazing, Winding, Rotor, Main.SLDASM", _
    ...
    swApp.ActivateDoc "Center, Fixture, Brazing, Winding, Rotor, Main"
    Set swModel = swApp.ActiveDoc
    ...
    swApp.ActivateDoc "Bottom Plate, Fixture, Brazing, Winding, Rotor, Main"
    Set swModel = swApp.ActiveDoc
    ...
    swApp.ActivateDoc "Assembly, Fixture, Brazing, Winding, Rotor, Main"
    Set swModel = swApp.ActiveDoc
...
'    Dim swApp As SldWorks.SldWorks
'
'    toolAssembleyPath = "C:\Users\FIlie\Documents\Felix Documents IPS\Master Tooling\Main rotor coils Brazing tool\Assembly, Fixture, Brazing, Winding, Rotor, Main.SLDASM"
'
'    'Generator Dimension - determine the distance between 2 coils when spread out on the fixture.
'    Dim RtoCoil As Double
'    Dim NumberCoils As Integer
